...
module Jnomad
  ...
  class Client
    ...
    def initialize(**args)
      ...
      port = args.fetch(:port, '4646')
      params = args.fetch(:params, {})
      uripath = URI::HTTP.build({:host => host, :port => port, :path => API_VERSION})
      @rest = RestClient::Resource.new(uripath, params)
    end
    ...
    def job_summary(id)
      path = File.join("/job", id, "/summary")
      return get(path)
    end

    #GET /v1/job/<ID>/deployments
    def job_deployments(id)
      path = File.join("/job", id, "/deployments")
      return get(path)
    end
